...
export class ShopService {
  ...
  async quoteDeliveryFee(customerLat: number, customerLng: number) {
    ...
    // Typed as `unknown`, not `number` — the real Prisma-generated type
    // for a Decimal-typed column is a Decimal object, not a plain
    // number, and `unknown` is safe for both since Number() accepts it.
    const zone = zones.find((z: { maxDistanceKm: unknown }) => distanceKm <= Number(z.maxDistanceKm));
